...
## 📋 Table Structure - All Tables and Columns
...
| user_id | INT | NOT NULL, UNIQUE, FK → users(user_id) | Links to users table |
...
| user_id | INT | NOT NULL, FK → users(user_id) | Links to users table |
...
| photographer_id | INT | NOT NULL, FK → photographers(photographer_id) | Links to photographers |
...
| creator_id | INT | NOT NULL, FK → users(user_id) | Event creator |
| category_id | INT | NOT NULL, FK → event_categories(category_id) | Event category |
...
| customer_id | INT | NOT NULL, FK → users(user_id) | Customer making booking |
| photographer_id | INT | NOT NULL, FK → photographers(photographer_id) | Booked photographer |
| event_id | INT | FK → events(event_id), ON DELETE SET NULL | Related event (nullable) |
...
| booking_id | INT | NOT NULL, FK → bookings(booking_id) | Links to booking |
...
| booking_id | INT | NOT NULL, UNIQUE, FK → bookings(booking_id) | Links to booking |
| photographer_id | INT | NOT NULL, FK → photographers(photographer_id) | Reviewed photographer |
| customer_id | INT | NOT NULL, FK → users(user_id) | Reviewer |
...
| user_id | INT | NOT NULL, FK → users(user_id) | Post author |
...
| post_id | INT | NOT NULL, FK → posts(post_id) | Links to post |
| user_id | INT | NOT NULL, FK → users(user_id) | User who liked |
...
| parent_comment_id | INT | FK → post_comments(comment_id) | Parent comment for replies |
...
| user_id | INT | NOT NULL, FK → users(user_id) | Collection creator |
...
| sender_id | INT | NOT NULL, FK → users(user_id) | Message sender |
| receiver_id | INT | FK → users(user_id), NULL for group messages | Message receiver (nullable) |
| group_id | INT | FK → community_groups(group_id), NULL for direct messages | Group ID for group chats (nullable) |
...
| user_id | INT | NOT NULL, FK → users(user_id) | Topic creator |
...
| topic_id | INT | NOT NULL, FK → discussion_topics(topic_id) | Links to topic |
| user_id | INT | NOT NULL, FK → users(user_id) | Reply author |
| reply_text | TEXT | NOT NULL | Reply text |
| parent_reply_id | INT | FK → discussion_replies(reply_id) | Parent reply for nested replies |
| likes_count | INT | DEFAULT 0 | Likes count |
| is_edited | BOOLEAN | DEFAULT FALSE | Edited flag |
| is_active | BOOLEAN | DEFAULT TRUE | Active status |
| created_at | TIMESTAMP | DEFAULT CURRENT_TIMESTAMP | Creation timestamp |
...
| creator_id | INT | NOT NULL, FK → users(user_id) | Group creator |
...
| group_id | INT | NOT NULL, FK → community_groups(group_id) | Links to group |
| user_id | INT | NOT NULL, FK → users(user_id) | Group member |
...
| user_id | INT | NOT NULL, FK → users(user_id) | Collaboration creator |
...
| collaboration_id | INT | NOT NULL, FK → collaborations(collaboration_id) | Links to collaboration |
| user_id | INT | NOT NULL, FK → users(user_id) | Responder |
...
| follower_id | INT | NOT NULL, FK → users(user_id) | User who follows |
| following_id | INT | NOT NULL, FK → users(user_id) | User being followed |
...
3. **users** ← photographers (1:1)
4. **users** ← events (1:N, creator)
5. **users** ← bookings (1:N, customer)
6. **users** ← posts (1:N)
7. **users** ← messages (1:N, sender/receiver)
8. **users** ← notifications (1:N)
9. **users** ← discussion_topics (1:N)
10. **users** ← discussion_replies (1:N)
11. **users** ← community_groups (1:N, creator)
12. **users** ← collaborations (1:N)
13. **photographers** ← photographer_portfolios (1:N)
14. **photographers** ← photographer_availability (1:N)
15. **photographers** ← bookings (1:N)
16. **photographers** ← booking_reviews (1:N)
17. **event_categories** ← events (1:N)
18. **events** ← bookings (1:N)
19. **bookings** ← booking_payments (1:N)
20. **bookings** ← booking_reviews (1:1)
21. **posts** ← post_likes (1:N)
22. **posts** ← post_comments (1:N)
23. **posts** ← photographer_portfolios (1:N, via post_id)
24. **post_comments** ← post_comments (1:N, parent comments)
25. **discussion_topics** ← discussion_replies (1:N)
26. **discussion_replies** ← discussion_replies (1:N, nested replies)
27. **community_groups** ← group_members (1:N)
28. **community_groups** ← messages (1:N, group chats)
29. **collaborations** ← collaboration_responses (1:N)
30. **users** ← follows (N:N, follower/following)

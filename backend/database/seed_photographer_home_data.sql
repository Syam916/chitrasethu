-- Seed data for Photographer Home Page
-- This file contains INSERT statements for bookings, booking requests, and related data
-- Run this file after ensuring you have photographers and customers in your database

-- Note: This assumes you have:
-- 1. At least one photographer user (user_id with photographer type)
-- 2. At least 3-4 customer users
-- 3. Event categories exist

-- Get photographer user_id (assuming first photographer)
DO $$
DECLARE
...
BEGIN
    ...
    IF photographer_user_id IS NOT NULL AND photographer_id IS NOT NULL THEN
        ...
        (
            customer_user_id_1, photographer_id, '2024-03-15', '10:00:00', 8,
        ...
        (
            customer_user_id_2, photographer_id, '2024-02-28', '14:00:00', 6,
        ...
        (
            customer_user_id_3, photographer_id, '2024-03-10', '06:00:00', 4,
        ...
        (
            customer_user_id_4, photographer_id, '2024-02-25', '11:00:00', 5,
        ...
        (
            customer_user_id_3, photographer_id, '2024-03-05', '06:00:00', 8,
        ...
        (
            customer_user_id_1, photographer_id, '2024-03-12', '17:00:00', 4,
            ...
            NOW() - INTERVAL '3 days', NOW() - INTERVAL '7 days', NOW() - INTERVAL '3 days'
        )
        ON CONFLICT DO NOTHING;

        -- Insert CURRENT bookings (in_progress)
        INSERT INTO bookings (
            customer_id, photographer_id, booking_date, booking_time, duration_hours,
            location, venue_name, event_type, total_amount, advance_amount, pending_amount,
            currency, status, payment_status, special_requirements, confirmed_at, created_at, updated_at
        ...
        (
            customer_user_id_1, photographer_id, CURRENT_DATE - INTERVAL '30 days', '11:00:00', 12,
            ...
            NOW() - INTERVAL '35 days', NOW() - INTERVAL '30 days', NOW() - INTERVAL '40 days', NOW() - INTERVAL '30 days'
        ),
        -- Booking 6: Fashion Forward Magazine - Fashion Shoot (past, completed)
        (
            customer_user_id_4, photographer_id, CURRENT_DATE - INTERVAL '45 days', '10:00:00', 6,
            'Studio 9, Mumbai', 'Studio 9', 'Fashion Shoot',
            40000.00, 40000.00, 0, 'INR', 'completed', 'paid',
            'Fashion editorial shoot for magazine',
            NOW() - INTERVAL '50 days', NOW() - INTERVAL '45 days', NOW() - INTERVAL '55 days', NOW() - INTERVAL '45 days'
        )
        ON CONFLICT DO NOTHING;
